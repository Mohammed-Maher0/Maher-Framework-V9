...
cat mining/urls_*.txt 2>/dev/null | sort -u > mining/urls_raw.txt
...
if command -v uro &>/dev/null; then
    ...
    echo "    > [!] uro not installed — using sort -u (install: pip install uro)"
fi

# ============================================================
# PHASE 3: SCOPE FILTERING
# ============================================================
echo "[+] 3. Scope Filtering..."
if [ -n "$SCOPE_FILE" ] && [ -f "$SCOPE_FILE" ]; then
    TOTAL_BEFORE=$(wc -l < all_urls.txt)
    # بناء pattern من الـ scope file
    SCOPE_PATTERN=$(cat "$SCOPE_FILE" | sed 's/\./\\./g' | paste -sd '|')
    grep -E "$SCOPE_PATTERN" all_urls.txt > all_urls_scoped.txt
    mv all_urls_scoped.txt all_urls.txt
    echo "    > Scope filter: $TOTAL_BEFORE → $(wc -l < all_urls.txt) URLs"
else
    # فلتر تلقائي على الـ root domain
    grep "$ROOT_DOMAIN" all_urls.txt > all_urls_filtered.txt
    mv all_urls_filtered.txt all_urls.txt
    echo "    > Auto-scope to $ROOT_DOMAIN: $(wc -l < all_urls.txt) URLs"
fi
