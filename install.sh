...
GF_DIR="$HOME/.gf"
mkdir -p "$GF_DIR" "$HOME/.config/gf"

info "gf patterns (community)"
git clone --quiet https://github.com/1ndianl33t/Gf-Patterns.git /tmp/gf-patterns 2>/dev/null
cp /tmp/gf-patterns/*.json "$GF_DIR/" 2>/dev/null
cp /tmp/gf-patterns/*.json "$HOME/.config/gf/" 2>/dev/null
